...
PlayerRenderer::PlayerRenderer(std::vector<Character> &characters, Renderer &renderer) : characters(characters),
                                                                                         renderer(renderer) {
    loadTextures();
}
...
void PlayerRenderer::loadTextures() {
    ...
    std::string actions[] = {"Attack", "Dead", "Hurt",
                             "Idle", "Recharge", "Run",
                             "Shot_1", "Shot_2", "Walk"};
    ...
    for (auto &character_type: character_types) {
        for (auto &action: actions) {
            std::string path = RESOURCE_PATH "/" + character_type + "/" + action + ".png";
    ...
    std::string zombie_actions[] = {"Attack_1", "Dead", "Hurt", "Idle", "Run", "Walk"};
...
void PlayerRenderer::render(int it) {
    for (auto &character: characters) {
        ...
        Rect srcRect = Rect(frame * frameSize, 0, frameSize, frameSize);
        Rect dstRect = Rect(x, y, frameSize, frameSize);

        renderer.Copy(*texture, srcRect, dstRect, 0, NullOpt,
                      character.getFlip());
    }
}